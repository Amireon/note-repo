...
关联性分析：是否套用第二问建立的分类模型？
- 不用，相关性分析：将附件表单2的数据按照玻璃类型以及是否风化分为4类（对于是否使用亚类进行分类由于分类后数据过少暂时持保留态度），然后建立相关性分析模型，从而得到各个化学成分之间的相关系数，通过纵向对比不用类比之间相关系数，来描述不同类别之间的差异性
- 
...
if __name__ == '__main__':
	data_mat = mat(load_data("data/XXX.txt"))
