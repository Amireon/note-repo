...
指令周期的概念：CPU取出并执行一条指令花费的全部时间
- 取值周期（取指令）、间址周期（取有效地址）、执行周期（取操作数）、中断周期（保存程序断点）
...
流水线中的多发技术
- 超标量技术：每个时钟周期内同时并发读条独立指令，并发执行
- 超流水线技术：将流水线寄存器插入流水线段中
- 超长指令字技术

- 中断源
- 中断判优：硬件排队、软件排队
- 中断向量：中断服务程序入口地址
- 中断响应的条件和时间：指令执行周期结束后

中断响应周期硬件（中断隐指令）完成的三件事情：
- 保存程序断点
- 寻找中断服务程序入口地址
- 关中断
