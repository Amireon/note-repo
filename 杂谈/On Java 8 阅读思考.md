...
## 第6章 初始化和清理
清理在C++中表现得尤为突出，C++需要程序员自己分配、回收内存，是诸多新手的噩梦，常常造成内存泄露

Java继承了C++构造器的概念，还额外引入了垃圾回收机制(Garbage Collector,GC)去自动回收不再被使用的对象所占的资源。

想要深入了解GC的工作过程与原理，那不得不讲解Java虚拟机(Java Virtual Machine,JVM)
...
但是，一旦你不在乎返回值，例如直接这样使用`f();`，那么Java 编译器就不知道究竟要调用哪个方法，读者也会相当困惑。尽管为了支持新特性，Java 8 在一些具体情形下提高了猜测的准确度，但是通常来说并不起作用。

调用一个方法且忽略返回值。这叫做调用一个函数的副作用，因为你不在乎返回值，只是想利用方法做些事。
...
### 数组初始化
持有的是数组的引用，因此做出的任何修改，都将作用到实际的数组对象上
